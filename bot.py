...
def short_id(oid): return str(oid)[-6:].upper()

async def get_user(uid):
    return await users.find_one({"user_id": uid})

async def upsert_user(uid, uname, nick=None):
    upd = {"$set": {"username": uname, "last_seen": now()},
           "$setOnInsert": {"user_id": uid, "balance": 0, "joined": now(), "roblox_nick": ""}}
    if nick:
...
# deposits
async def add_deposit(uid, uname, nick, amount):
    r = await deposits.insert_one({"user_id": uid, "username": uname, "roblox_nick": nick,
                                    "amount": amount, "status": "pending", "created_at": now()})
    return r.inserted_id

async def get_deposit(did):
    return await deposits.find_one({"_id": ObjectId(did)})

async def approve_deposit(did):
    dep = await deposits.find_one({"_id": ObjectId(did)})
    if dep:
        await deposits.update_one({"_id": ObjectId(did)}, {"$set": {"status": "approved"}})
        await users.update_one({"user_id": dep["user_id"]}, {"$inc": {"balance": dep["amount"]}})

async def reject_deposit(did):
    await deposits.update_one({"_id": ObjectId(did)}, {"$set": {"status": "rejected"}})

# orders
async def add_order(uid, uname, nick, robux, price):
    r = await orders.insert_one({"user_id": uid, "username": uname, "roblox_nick": nick,
                                  "robux_amount": robux, "price_sum": price, "status": "pending", "created_at": now()})
    return r.inserted_id

async def get_order(oid):
    return await orders.find_one({"_id": ObjectId(oid)})

async def approve_order(oid):
    await orders.update_one({"_id": ObjectId(oid)}, {"$set": {"status": "approved"}})

async def reject_order(oid):
    o = await orders.find_one({"_id": ObjectId(oid)})
    if o and o["status"] == "pending":
        await orders.update_one({"_id": ObjectId(oid)}, {"$set": {"status": "rejected"}})
...
async def add_trade(uid, uname, nick, name, bio, photo_id):
    r = await trades.insert_one({"user_id": uid, "username": uname, "roblox_nick": nick,
                                  "name": name, "bio": bio, "photo_id": photo_id,
                                  "status": "active", "created_at": now()})
    return r.inserted_id

async def get_trade(tid):
    return await trades.find_one({"_id": ObjectId(tid)})
...
async def edit_trade(tid, name, bio):
    await trades.update_one({"_id": ObjectId(tid)}, {"$set": {"name": name, "bio": bio}})

async def delete_trade(tid):
    await trades.update_one({"_id": ObjectId(tid)}, {"$set": {"status": "deleted"}})

# sales
async def add_sale(uid, uname, nick, name, photo_id, currency, price):
    r = await sales.insert_one({"user_id": uid, "username": uname, "roblox_nick": nick,
                                 "name": name, "photo_id": photo_id, "currency": currency,
                                 "price": price, "status": "active", "created_at": now()})
    return r.inserted_id

async def get_sale(sid):
    return await sales.find_one({"_id": ObjectId(sid)})
...
async def edit_sale(sid, name, price):
    await sales.update_one({"_id": ObjectId(sid)}, {"$set": {"name": name, "price": price}})

async def delete_sale(sid):
    await sales.update_one({"_id": ObjectId(sid)}, {"$set": {"status": "deleted"}})
...
def main_kb():
    ...
    b.button(text="➕ Sotish qo'shish");   b.button(text="💬 Chat")
    b.adjust(2,2,2,2)
    return b.as_markup(resize_keyboard=True)

def cancel_kb():
    b = ReplyKeyboardBuilder(); b.button(text="❌ Bekor qilish")
    return b.as_markup(resize_keyboard=True)
...
async def is_sub(uid):
    try:
        ...
        logging.error(f"Sub check xato: {e}"); return False
...
async def _send_or_edit(cb: types.CallbackQuery, photo_id, text, markup):
    try:
        ...
        try:
            if photo_id: await cb.message.answer_photo(photo_id, caption=text, reply_markup=markup)
...
async def cmd_start(msg: types.Message, state: FSMContext):
    uid = msg.from_user.id
    if not await is_sub(uid):
        await msg.answer("👋 Salom! Botdan foydalanish uchun avval kanalimizga obuna bo'ling!", reply_markup=sub_kb()); return
    await upsert_user(uid, msg.from_user.username or "user")
    u = await get_user(uid)
    if not u or not u.get("roblox_nick"):
        await msg.answer("📝 Roblox nickingizni yozing:", reply_markup=cancel_kb())
...
async def cb_my_trades(cb: types.CallbackQuery):
    ...
    if not items: await cb.answer("Faol trade e'lonlaringiz yo'q!", show_alert=True); return
...
async def cb_my_sales(cb: types.CallbackQuery):
    ...
    if not items: await cb.answer("Faol sotuv e'lonlaringiz yo'q!", show_alert=True); return
    page = max(0, min(page, len(items)-1))
    s = items[page]
...
async def cb_damt(cb: types.CallbackQuery, state: FSMContext):
    ...
    if cb.data == "damt_custom":
        ...
        await state.set_state(Dep.custom_amount); await cb.answer(); return
...
async def cb_dep_paid(cb: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(Dep.check_photo); await cb.answer()
...
async def cb_dep_cancel(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("❌ Bekor qilindi.", reply_markup=main_kb()); await cb.answer()

@dp.message(Dep.check_photo, F.photo)
async def dep_check_photo(msg: types.Message, state: FSMContext):
    uid = msg.from_user.id; uname = msg.from_user.username or "user"
    u = await get_user(uid); d = await state.get_data()
    amount = d.get("dep_amount", 0); photo_id = msg.photo[-1].file_id
    nick = u.get("roblox_nick", "-") if u else "-"
    did = await add_deposit(uid, uname, nick, amount)
    b = InlineKeyboardBuilder()
    b.button(text="✅ Tasdiqlash", callback_data=f"dok_{did}")
    b.button(text="❌ Rad etish",  callback_data=f"dno_{did}")
    b.adjust(2)
    try:
        await bot.send_photo(ADMIN_ID, photo_id, caption=(
            f"💰 *To'lov #{short_id(did)}*\n\n"
            f"👤 @{uname} (`{uid}`)\n🎮 Roblox: *{nick}*\n"
            f"💵 Miqdor: *{amount:,} so'm*\n🕐 {now()}"
        ), reply_markup=b.as_markup())
    except Exception as e: logging.error(f"Admin ga xato: {e}")
    await state.clear()
...
async def cb_dok(cb: types.CallbackQuery):
    ...
    if not dep or dep["status"] != "pending": await cb.answer("Allaqachon ko'rilgan!", show_alert=True); return
    await approve_deposit(did)
    try: await bot.send_message(dep["user_id"], f"✅ To'lovingiz tasdiqlandi!\n💰 *{dep['amount']:,} so'm* hisobingizga qo'shildi!", reply_markup=main_kb())
    except: pass
    try: await cb.message.edit_caption(cb.message.caption + f"\n\n✅ TASDIQLANDI ({now()})")
    except: pass
...
async def cb_dno(cb: types.CallbackQuery):
    ...
    try: await bot.send_message(dep["user_id"], f"❌ To'lovingiz rad etildi.\n📋 #{short_id(ObjectId(did))}\n\nAdmin bilan bog'laning.", reply_markup=main_kb())
    except: pass
    try: await cb.message.edit_caption(cb.message.caption + f"\n\n❌ RAD ETILDI ({now()})")
    except: pass
...
async def cmd_buy(msg: types.Message, state: FSMContext):
    if not await check_access(msg, state): return
    uid = msg.from_user.id; bal = await get_balance(uid)
...
async def cb_buy(cb: types.CallbackQuery):
    uid = cb.from_user.id
    if not await is_sub(uid): await cb.answer("❌ Avval kanalga obuna bo'ling!", show_alert=True); return
    u = await get_user(uid)
    if not u or not u.get("roblox_nick"): await cb.answer("❌ Avval /start yozing!", show_alert=True); return
    robux = int(cb.data.split("_")[1]); price = price_for(robux); bal = await get_balance(uid)
    if bal < price:
...
async def cb_ook(cb: types.CallbackQuery):
    ...
    oid = cb.data.split("_")[1]; o = await get_order(oid)
    if not o or o["status"] != "pending": await cb.answer("Allaqachon ko'rilgan!", show_alert=True); return
    await approve_order(oid)
    try: await bot.send_message(o["user_id"], f"🎉 *{o['robux_amount']} Robux* Roblox accountingizga yuborildi!\n📋 Buyurtma #{short_id(ObjectId(oid))}", reply_markup=main_kb())
    except: pass
...
async def cb_ono(cb: types.CallbackQuery):
    ...
    try: await bot.send_message(o["user_id"], f"❌ Buyurtma #{short_id(ObjectId(oid))} rad etildi.\n💰 {o['price_sum']:,} so'm qaytarildi.", reply_markup=main_kb())
    except: pass
...
async def cmd_trades(msg: types.Message, state: FSMContext):
    ...
    if not items: await msg.answer("🔄 Hozircha faol tradelar yo'q.\n\n➕ *Trade qo'shish* tugmasini bosing!"); return
...
async def _send_trade_page(target, items, page, is_cb=True):
    ...
    if uname: b.button(text="💬 Murojaat", url=f"https://t.me/{uname}")
    b.adjust(2, 1)
    if is_cb:
...
async def cb_tp(cb: types.CallbackQuery):
    page = int(cb.data.split("_")[1]); items = await active_trades()
    if not items: await cb.answer("Tradelar yo'q!", show_alert=True); return
    page = max(0, min(page, len(items)-1))
    await _send_trade_page(cb, items, page); await cb.answer()
...
async def ta_name(msg: types.Message, state: FSMContext):
    if msg.text == "❌ Bekor qilish": await state.clear(); await msg.answer("Bekor qilindi.", reply_markup=main_kb()); return
    if len(msg.text.strip()) < 5: await msg.answer("❌ Sarlavha kamida 5 ta belgi bo'lsin:"); return
...
async def ta_photo(msg: types.Message, state: FSMContext):
    d = await state.get_data(); uid = msg.from_user.id; uname = msg.from_user.username or "user"
    u = await get_user(uid); photo_id = msg.photo[-1].file_id
    tid = await add_trade(uid, uname, u.get("roblox_nick",""), d["t_name"], d["t_bio"], photo_id)
    await state.clear()
    try: await bot.send_photo(ADMIN_ID, photo_id, caption=f"🔄 Yangi trade #{short_id(tid)}\n👤 @{uname}\n📦 {d['t_name']}\n📝 {d['t_bio']}")
    except: pass
...
async def ta_no_photo(msg: types.Message, state: FSMContext):
    ...
    await state.clear()
    try: await bot.send_message(ADMIN_ID, f"🔄 Yangi trade #{short_id(tid)}\n👤 @{uname}\n📦 {d['t_name']}\n📝 {d['t_bio']}")
    except: pass
    await msg.answer(f"✅ Trade e'lon qilindi! *#{short_id(tid)}*", reply_markup=main_kb())

@dp.callback_query(F.data.startswith("etrade_"))
async def cb_etrade(cb: types.CallbackQuery, state: FSMContext):
    tid = cb.data.split("_")[1]; t = await get_trade(tid)
...
async def cmd_sales(msg: types.Message, state: FSMContext):
    ...
    if not items: await msg.answer("📊 Hozircha sotuvdagi buyumlar yo'q.\n\n➕ *Sotish qo'shish* tugmasini bosing!"); return
...
async def cb_sp(cb: types.CallbackQuery):
    page = int(cb.data.split("_")[1]); items = await active_sales()
    if not items: await cb.answer("Sotuvlar yo'q!", show_alert=True); return
    page = max(0, min(page, len(items)-1))
    await _send_sale_page(cb, items, page); await cb.answer()
...
async def sa_no_photo(msg: types.Message, state: FSMContext):
    if msg.text == "❌ Bekor qilish": await state.clear(); await msg.answer("Bekor qilindi.", reply_markup=main_kb()); return
    await state.update_data(s_photo=None); await _ask_currency(msg, state)
...
async def cb_sc(cb: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SaleAdd.price); await cb.answer()

@dp.message(SaleAdd.price)
async def sa_price(msg: types.Message, state: FSMContext):
    if msg.text == "❌ Bekor qilish": await state.clear(); await msg.answer("Bekor qilindi.", reply_markup=main_kb()); return
    txt = msg.text.strip().replace(" ","").replace(",","")
    if not txt.isdigit(): await msg.answer("❌ Faqat raqam kiriting:"); return
    d = await state.get_data(); uid = msg.from_user.id; uname = msg.from_user.username or "user"
    u = await get_user(uid)
    ...
    try:
        ...
        if d.get("s_photo"): await bot.send_photo(ADMIN_ID, d["s_photo"], caption=cap)
        else:                 await bot.send_message(ADMIN_ID, cap)
    except: pass
    await msg.answer(f"✅ Sotuv e'lon qilindi! *#{short_id(sid)}*\n📦 {d['s_name']}\n💰 {int(txt):,} {d['s_currency']}", reply_markup=main_kb())

@dp.callback_query(F.data.startswith("esale_"))
async def cb_esale(cb: types.CallbackQuery, state: FSMContext):
    sid = cb.data.split("_")[1]; s = await get_sale(sid)
...
async def esale_price(msg: types.Message, state: FSMContext):
    ...
    if not txt.isdigit(): await msg.answer("❌ Faqat raqam:"); return
    d = await state.get_data()
...
async def cmd_admin(msg: types.Message):
    if msg.from_user.id != ADMIN_ID: await msg.answer("❌ Ruxsat yo'q!"); return
    tr = await active_trades(); sl = await active_sales(); or_ = await pending_orders(); cnt = await users_count()
    b = InlineKeyboardBuilder()
...
async def adm_ord(cb: types.CallbackQuery):
    ...
    if not ol: await cb.answer("Kutayotgan buyurtmalar yo'q!", show_alert=True); return
    for o in ol:
        b = InlineKeyboardBuilder()
...
async def adm_tr(cb: types.CallbackQuery):
    ...
    for t in items[:10]:
        ...
        if t.get("photo_id"): await cb.message.answer_photo(t["photo_id"], caption=caption, reply_markup=b.as_markup())
        else:                  await cb.message.answer(caption, reply_markup=b.as_markup())
    await cb.answer()
...
async def adm_sl(cb: types.CallbackQuery):
    ...
    for s in items[:10]:
        ...
        if s.get("photo_id"): await cb.message.answer_photo(s["photo_id"], caption=caption, reply_markup=b.as_markup())
        else:                  await cb.message.answer(caption, reply_markup=b.as_markup())
    await cb.answer()
...
async def adm_addbal(cb: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminCmd.add_balance); await cb.answer()
...
async def admin_addbalance(msg: types.Message, state: FSMContext):
    ...
    if len(parts) != 2 or not parts[0].isdigit() or not parts[1].isdigit():
        await msg.answer("❌ Format: `<user_id> <summa>`"); return
...
async def bc_text(msg: types.Message, state: FSMContext):
    if msg.text == "❌ Bekor qilish": await state.clear(); await msg.answer("Bekor qilindi.", reply_markup=main_kb()); return
    d = await state.get_data(); text = msg.text.strip(); photo = d.get("bc_photo")
    await state.clear()
    uids = await all_user_ids(); sent = 0
    for uid in uids:
        try:
